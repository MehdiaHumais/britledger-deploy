...
function notesToHtml(s: string): string {
...
export function DocumentBuilder({ type, initialNumber, initialData, onSave }: DocumentBuilderProps) {
  ...
  const [items, setItems] = useState<LineItem[]>(() => {
    const initial = Array.isArray(initialData?.items) ? initialData.items : []
    if (initial.length > 0) {
      return initial.map((i: any) => ({
    ...
    return [{ id: '1', description: '', quantity: 1, unitPrice: 0, taxRate: 20 }]
  })
  ...
  const handlePreviewPDF = () => {
    ...
    const html = `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"/><meta name="viewport" content="width=device-width,initial-scale=1.0"/><title>${docTitle} ${documentNumber}</title><style>
      *{margin:0;padding:0;box-sizing:border-box}
      ...
      .status{display:inline-block;padding:3px 10px;border-radius:99px;font-size:11px;font-weight:700;margin-top:5px}
      ...
      ${notesHtml ? '<div class="notes"><h4>Notes</h4><p>'+notesHtml+'</p></div>' : ''}
    ...
    <div className="grid gap-6 lg:grid-cols-3">
      ...
      {pdfPreviewOpen && (
        <Dialog open={pdfPreviewOpen} onOpenChange={() => setPdfPreviewOpen(false)}>
          <DialogContent className="max-w-4xl w-[95vw] h-[90vh] max-h-[90vh] flex flex-col p-4 sm:p-6">
            <div className="flex justify-between items-center mb-4 flex-shrink-0">
              <DialogTitle className="text-xl font-bold">{type === 'invoice' ? 'Invoice' : 'Quotation'} {documentNumber}</DialogTitle>
